...
@if ($get == 'users' && !!$lastMessage)
    ...
    <table class="messenger-list-item" data-contact="{{ $user->id }}">
        ...
        style="background-image: url('{{ $user->avatar ?  $user->avatar : asset('assets/img/user/avatar.png') }}');">
        </div> --}}
                {{-- @if ($user->avatar)
        <img src="{{ $user->avatar}}" width="50" class="rounded-circle" height="50" alt="{{$user->name}} avatar">
    @else
    ...
    @endif --}}
                {{-- @php
                    $user = Auth::user();
                @endphp --}}
                <img src="{{ $user->avatar }}" width="50" class="rounded-circle"
                    height="50" alt="{{ $user->name }} avatar">
